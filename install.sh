...
get_latest_version() {
    ...
    if [ -n "${GITHUB_TOKEN:-}" ]; then
        use_auth="yes"
        debug "Using GITHUB_TOKEN for API authentication"
    fi
    ...
    if command -v curl &> /dev/null; then
        ...
        if [ "$use_auth" = "yes" ]; then
            response=$(curl -fsSL -H "Authorization: Bearer $GITHUB_TOKEN" "$api_url" 2>&1) || {
        ...
        local response
        if [ "$use_auth" = "yes" ]; then
            response=$(wget -qO- --header="Authorization: Bearer $GITHUB_TOKEN" "$api_url" 2>&1) || {
